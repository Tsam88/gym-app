<template>
    ...
    <div>
        <div class="mb-3">
            <h1 class="h1 d-inline align-middle">Create Excluded Calendar Dates</h1>
        </div>
...
<script>
    export default {
        ...
        methods:{
            submitForm() {
                axios.post('/admin/excluded-calendar-dates', this.form)
                    .then((result) => {
                        //Perform Success Action
                        this.$router.push({ name: 'ShowExcludedCalendarDates' });

                        // display success message
                        this.$alertHandler.showAlert('Excluded calendar dates created successfully', result.status);
                    })
                    .catch((error) => {
